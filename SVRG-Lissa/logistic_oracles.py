...
class DataHolder:
	...
	def load_mnist_49(self):
		f = open('C:/Users/wangjing computer/PycharmProjects/kfac/pythonProject/Data/mnist49data', 'rb')
	...
	def logistic_batch_grad(self, data_batch, model):
		...
		for data_indiv in data_batch:
			batch_grad = batch_grad + self.logistic_indiv_grad(data_indiv, model)
	...
	def logistic_batch_hess_full(self, data_batch, model):
		...
		for data_indiv in data_batch:
			batch_hess = batch_hess + np.outer(self.logistic_indiv_hess(data_indiv, model), self.logistic_indiv_hess(data_indiv, model))
